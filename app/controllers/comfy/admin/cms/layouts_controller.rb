...
class Comfy::Admin::Cms::LayoutsController < Comfy::Admin::Cms::BaseController
  ...
  def index
    ...
    @layouts = if params[:categories].present?
      @site.layouts.roots.includes(:categories).for_category(params[:categories]).order(:position)
    else
      @site.layouts.roots.order(:position)
    end
  end
