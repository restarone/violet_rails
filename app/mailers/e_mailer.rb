class EMailer < ApplicationMailer
  ...
  def ship
    ...
    message_id = @message_thread.email_message_id ? @message_thread.email_message_id : "#{Digest::SHA2.hexdigest(Time.now.to_i.to_s)}@#{Rails.application.config.action_mailer.mailgun_settings[:domain]}"
